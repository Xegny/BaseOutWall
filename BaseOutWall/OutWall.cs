...
namespace BaseOutWall
{
    public class OutWall
    {
        ...
        public static void CreateOutWall(WallData data)
        {
            #region 预操作
            DocumentLock m_DocumentLock = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument.LockDocument();
            Document doc = Application.DocumentManager.MdiActiveDocument;
            Database db = doc.Database;
            Editor editor = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument.Editor;
            List<int> changedIndices = OutWall.FindChangedWallThicknessIndices(data.storyList);//i和i+1层
            #endregion
            ...
            #region 画外墙
            // 使用 LINQ 求和 StoryHeight 属性
            double totalHeight = data.storyList.Sum(story => double.Parse(story.StoryHeight));
            //1
            Line outwallline1 = new Line(
                point + new Vector3d(0, -300, 0),
                point + new Vector3d(0, totalHeight + data.dibanhou + data.dianceng, 0));
            outwallline1.Layer = "0S-WALL-LINE";
            //2外
            Line outwallline2 = new Line(
                point + new Vector3d(-data.waiqiangkuan, -300, 0),
                point + new Vector3d(-data.waiqiangkuan, totalHeight + data.dibanhou + data.dianceng, 0));
            outwallline2.Layer = "0S-WALL-LINE";
            //zdx
            Polyline waiqiangzdx = ZheDuanXian(point + new Vector3d(-data.waiqiangkuan, -300, 0),
                point + new Vector3d(0, -300, 0));
            //3
            Polyline thickoutwallline = new Polyline();
            thickoutwallline.AddVertexAt(0, new Point2d(point.X, point.Y - 150), 0, 0, 0);
            thickoutwallline.AddVertexAt(1, new Point2d(point.X, point.Y + totalHeight + data.dibanhou + data.dianceng), 0, 0, 0);
            thickoutwallline.ConstantWidth = 15;
            thickoutwallline.Layer = "0S-WALL-LINE";
            //标注
            Point3d waiqiangbiaozhudian1 = point + new Vector3d(0, totalHeight + data.dibanhou + data.dianceng, 0);
            Point3d waiqiangbiaozhudian2 = point + new Vector3d(-data.waiqiangkuan, totalHeight + data.dibanhou + data.dianceng, 0);
            OutWall.DrawBiaoZhu(db, waiqiangbiaozhudian1, waiqiangbiaozhudian2, -200);

            #endregion

            #region 旁边的标注(totalHeight)
            double dicenggao = Double.Parse(data.storyList[data.storyList.Count - 1].StoryHeight);
            double dingcenggao = Double.Parse(data.storyList[0].StoryHeight);
            double distance = 50;
            //"室内"            
            MText mtextSN = new MText();//数字
            mtextSN.SetDatabaseDefaults();
            ...
            mtextSN.TextHeight = 250;
            mtextSN.Attachment = AttachmentPoint.MiddleCenter;// 设置文本对象的插入点为文本的顶端中点
            mtextSN.Location = point + new Vector3d(data.neiqiangkuan * 1.5 + 900 + 1000 + 300, totalHeight * 2 / 5, 0);
            mtextSN.Layer = "0S-DETL-DIMS";
            Polyline shineikuang = DrawTextKuang(mtextSN, distance);
            shineikuang.Layer = "0S-DETL-DIMS";
            //"室外"
            ...
            mtextSW.TextHeight = 250;
            mtextSW.Attachment = AttachmentPoint.MiddleCenter;// 设置文本对象的插入点为文本的顶端中点
            mtextSW.Location = point + new Vector3d(-data.waiqiangkuan * 2 - 300, totalHeight * 2 / 5, 0);
            mtextSW.Layer = "0S-DETL-DIMS";
            Polyline shiwaikuang = DrawTextKuang(mtextSW, distance);//画个框
            shiwaikuang.Layer = "0S-DETL-DIMS";
            //"地下连续墙"
            ...
            mtextDXLXQ.TextHeight = 250;
            mtextDXLXQ.Attachment = AttachmentPoint.BottomRight;// 设置文本对象的插入点为文本的顶端中点
            mtextDXLXQ.Location = point + new Vector3d(-data.waiqiangkuan * 1.05, totalHeight * 4 / 7, 0);
            mtextDXLXQ.Layer = "0S-BEAM-TEXT";
            Line lineDXLXQ = new Line(point + new Vector3d(-data.waiqiangkuan - 960, totalHeight * 4 / 7, 0)//下划线
                , point + new Vector3d(-data.waiqiangkuan * 7 / 8, totalHeight * 4 / 7, 0));
            lineDXLXQ.Layer = "0S-BEAM-TEXT";
            #endregion

            #region 画内墙
            double banhou1 = double.Parse(data.storyList[0].SlabT);
            //顶端虚线
            Line inwallline2 = new Line(
                point + new Vector3d(data.neiqiangkuan, totalHeight + data.dibanhou + data.dianceng - banhou1, 0),
                point + new Vector3d(data.neiqiangkuan, totalHeight + data.dibanhou + data.dianceng, 0));
            inwallline2.Linetype = "HIDDEN";
            inwallline2.Layer = "0S-WALL-LINE";
            //顶层墙
            Line dingcengwall = new Line(
                point + new Vector3d(data.neiqiangkuan, totalHeight + data.dibanhou + data.dianceng - banhou1, 0),
                point + new Vector3d(data.neiqiangkuan, totalHeight + data.dibanhou + data.dianceng - dingcenggao, 0));
            dingcengwall.Layer = "0S-WALL-LINE";
            //水平配筋
            Point3d di_midshuipingpeijin1 = point + new Vector3d(40 + 5 + 20, totalHeight + data.dibanhou + data.dianceng - dingcenggao / 2, 0);
            Point3d di_midshuipingpeijin2 = point + new Vector3d(data.neiqiangkuan - (40 + 5 + 20), totalHeight + data.dibanhou + data.dianceng - dingcenggao / 2, 0);
            Point3d di_upshuipingpeijin1 = point + new Vector3d(40 + 5 + 20, totalHeight + data.dibanhou + data.dianceng - dingcenggao / 2 + 200, 0);
            Point3d di_upshuipingpeijin2 = point + new Vector3d(data.neiqiangkuan - (40 + 5 + 20), totalHeight + data.dibanhou + data.dianceng - dingcenggao / 2 + 200, 0);
            Point3d di_downshuipingpeijin1 = point + new Vector3d(40 + 5 + 20, totalHeight + data.dibanhou + data.dianceng - dingcenggao / 2 - 200, 0);
            Point3d di_downshuipingpeijin2 = point + new Vector3d(data.neiqiangkuan - (40 + 5 + 20), totalHeight + data.dibanhou + data.dianceng - dingcenggao / 2 - 200, 0);
            //
            Polyline di_midcircle1 = new Polyline();
            di_midcircle1.AddVertexAt(0, new Point2d(di_midshuipingpeijin1.X - 10, di_midshuipingpeijin1.Y), 1, 0, 0);
            di_midcircle1.AddVertexAt(1, new Point2d(di_midshuipingpeijin1.X + 10, di_midshuipingpeijin1.Y), 1, 0, 0);
            ...
            di_xiegujin.Layer = "0S-DETL-RBAR";
            //标注
            Point3d dingcengneiqiangbiaozhu1 = point + new Vector3d(0, totalHeight + data.dibanhou + data.dianceng, 0);
            Point3d dingcengneiqiangbiaozhu2 = point + new Vector3d(data.neiqiangkuan, totalHeight + data.dibanhou + data.dianceng, 0);
            OutWall.DrawBiaoZhu(db, dingcengneiqiangbiaozhu1, dingcengneiqiangbiaozhu2, 200);
            #endregion
            ...
            List<Line> insidewall = new List<Line>();
            List<Polyline> neiqiangpeijin = new List<Polyline>();
            for (int i = 1; i < data.storyList.Count; i++)
            {
                //参数
                double danyuanqiangkuan = Double.Parse(data.storyList[i].WallThickness);
                double qiangjiaogaodu = totalHeight + data.dianceng + data.dibanhou - (data.dingbiaogao * 1000 - Double.Parse(data.storyList[i].StoryElevation) * 1000);
                double danyuancenggao = Double.Parse(data.storyList[i].StoryHeight);
                //内墙线
                Line neiqiangxian = new Line(
                    point + new Vector3d(danyuanqiangkuan, qiangjiaogaodu + danyuancenggao, 0),
                    point + new Vector3d(danyuanqiangkuan, qiangjiaogaodu, 0));
                ...
                insidewall.Add(neiqiangxian);
                //转换连接线
                if (data.storyList[i - 1].WallThickness != data.storyList[i].WallThickness)
                {
                    double qianyiceng = Double.Parse(data.storyList[i - 1].WallThickness);
                    Line zhuanhuanxian = new Line(
                        point + new Vector3d(qianyiceng, qiangjiaogaodu + Double.Parse(data.storyList[i].StoryHeight), 0),
                        point + new Vector3d(danyuanqiangkuan, qiangjiaogaodu + Double.Parse(data.storyList[i].StoryHeight), 0));
                    zhuanhuanxian.Layer = "0S-WALL-LINE";
                    insidewall.Add(zhuanhuanxian);
                    //标注
                    Point3d neiqiangbiaozhu1 = point + new Vector3d(0, qiangjiaogaodu + Double.Parse(data.storyList[i].StoryHeight), 0);
                    Point3d neiqiangbiaozhu2 = point + new Vector3d(danyuanqiangkuan, qiangjiaogaodu + Double.Parse(data.storyList[i].StoryHeight), 0);
                    OutWall.DrawBiaoZhu(db, neiqiangbiaozhu1, neiqiangbiaozhu2, -300);
                }
                //水平配筋
                Point3d midshuipingpeijin1 = point + new Vector3d(40 + 5 + 20, qiangjiaogaodu + danyuancenggao / 2, 0);
            ...
            #region 底板
            double neiqiangkuan0 = data.xianeiqiangkuan;
            Line diban = new Line(
                point + new Vector3d(neiqiangkuan0, data.dibanhou + data.dianceng, 0),
                point + new Vector3d(neiqiangkuan0 + 900, data.dibanhou + data.dianceng, 0));//标高标志插入点
            diban.Layer = "0S-WALL-LINE";
            //标注
            Point3d dibanbiaozhudian1 = point + new Vector3d(neiqiangkuan0 + 900, data.dibanhou + data.dianceng, 0);
            Point3d dibanbiaozhudian2 = point + new Vector3d(neiqiangkuan0 + 900, data.dianceng, 0);
            OutWall.DrawBiaoZhu(db, dibanbiaozhudian1, dibanbiaozhudian2, 500);
            //"底板"
            MText mtextDB = new MText();//数字
            mtextDB.SetDatabaseDefaults();
            ...
            mtextDB.TextHeight = 200;
            mtextDB.Attachment = AttachmentPoint.MiddleCenter;// 设置文本对象的插入点为文本的顶端中点
            mtextDB.Location = point + new Vector3d(neiqiangkuan0 + 600, (data.dibanhou + data.dianceng) * 0.67, 0);
            mtextDB.Layer = "0S-BEAM-TEXT";
            //"LaE"
            MText mtextLaE = new MText();//数字
            mtextLaE.SetDatabaseDefaults();
            ...
            mtextLaE.TextHeight = 150;
            mtextLaE.Attachment = AttachmentPoint.BottomCenter;// 设置文本对象的插入点为文本的顶端中点
            mtextLaE.Location = point + new Vector3d((data.neiqiangkuan + 900) / 2 - 100, data.dianceng + 100, 0);
            mtextLaE.Layer = "0S-BEAM-TEXT";
            //折断线
            Polyline dibanzdx = ZheDuanXian(point + new Vector3d(neiqiangkuan0 + 900, 0, 0),
                point + new Vector3d(neiqiangkuan0 + 900, data.dibanhou + data.dianceng, 0));
            #region 画标高          
            Point3d pointbioagao1 = point + new Vector3d(neiqiangkuan0 + 900 + 300, data.dibanhou + data.dianceng, 0);
            //线
            Line biaogaodixian1 = new Line(pointbioagao1 + new Vector3d(-200, 0, 0), pointbioagao1 + new Vector3d(200, 0, 0));
            Line biaogaoshangxian1 = new Line(pointbioagao1 + new Vector3d(-200, 200, 0), pointbioagao1 + new Vector3d(1000, 200, 0));
            Line biaogaozuoxiexian1 = new Line(pointbioagao1, pointbioagao1 + new Vector3d(200, 200, 0));
            ...
            MText mtext1 = new MText();//数字
            mtext1.SetDatabaseDefaults();
            mtext1.Contents = data.storyList[data.storyList.Count - 1].StoryElevation + "(相对标高)";
            mtext1.TextHeight = 250;
            mtext1.Attachment = AttachmentPoint.MiddleCenter;// 设置文本对象的插入点为文本的顶端中点
            mtext1.Location = pointbioagao1 + new Vector3d(600, 400, 0);
            mtext1.Layer = "0S-BEAM-TEXT";
            MText mtext11 = new MText();//层号
            mtext11.SetDatabaseDefaults();
            mtext11.Contents = "B" + data.storyList.Count.ToString();
            mtext11.TextHeight = 250;
            mtext11.Attachment = AttachmentPoint.MiddleCenter;// 设置文本对象的插入点为文本的顶端中点
            mtext11.Location = pointbioagao1 + new Vector3d(600, 700, 0);
            mtext11.Layer = "0S-BEAM-TEXT";
            ...
            Line dingbanup = new Line(
                point + new Vector3d(-data.waiqiangkuan, totalHeight + data.dibanhou + data.dianceng, 0),
                point + new Vector3d(data.neiqiangkuan + 900, totalHeight + data.dibanhou + data.dianceng, 0));//标高标志插入点
            dingbanup.Layer = "0S-WALL-LINE";
            //标注
            Point3d dingbanbiaozhu1 = point + new Vector3d(data.neiqiangkuan + 600, totalHeight + data.dibanhou + data.dianceng, 0);
            Point3d dingbanbiaozhu2 = point + new Vector3d(data.neiqiangkuan + 600, totalHeight + data.dibanhou + data.dianceng - banhou1, 0);
            OutWall.DrawBiaoZhu(db, dingbanbiaozhu1, dingbanbiaozhu2, 0);

            #region 画标高          
            Point3d pointbioagao2 = point + new Vector3d(data.neiqiangkuan + 900 + 300, totalHeight + data.dibanhou + data.dianceng, 0);
            //线
            Line biaogaodixian2 = new Line(pointbioagao2 + new Vector3d(-200, 0, 0), pointbioagao2 + new Vector3d(200, 0, 0));
            Line biaogaoshangxian2 = new Line(pointbioagao2 + new Vector3d(-200, 200, 0), pointbioagao2 + new Vector3d(1000, 200, 0));
            Line biaogaozuoxiexian2 = new Line(pointbioagao2, pointbioagao2 + new Vector3d(200, 200, 0));
            ...
            Line dingbandown = new Line(
                point + new Vector3d(data.neiqiangkuan, totalHeight + data.dibanhou + data.dianceng - banhou1, 0),
                point + new Vector3d(data.neiqiangkuan + 900, totalHeight + data.dibanhou + data.dianceng - banhou1, 0));
            dingbandown.Layer = "0S-WALL-LINE";
            //折断线
            Polyline dingbanzdx = ZheDuanXian(point + new Vector3d(data.neiqiangkuan + 900, totalHeight + data.dibanhou + data.dianceng - banhou1, 0),
                point + new Vector3d(data.neiqiangkuan + 900, totalHeight + data.dibanhou + data.dianceng, 0));
            #endregion
            ...
            for (int i = 1; i < data.storyList.Count; i++)
            {
                double dingduanjuli = Math.Abs(double.Parse(data.storyList[i - 1].StoryElevation) * 1000 - data.dingbiaogao);
                double banhou = double.Parse(data.storyList[i].SlabT);

                data.neiqiangkuan = Double.Parse(data.storyList[i].WallThickness);
                //楼板的上边那条线
                Line uplouban = new Line(
                    point + new Vector3d(data.neiqiangkuan, totalHeight + data.dibanhou + data.dianceng - dingduanjuli, 0),
                    point + new Vector3d(data.neiqiangkuan + 900, totalHeight + data.dibanhou + data.dianceng - dingduanjuli, 0));
                uplouban.Layer = "0S-WALL-LINE";
                uploubanlist.Add(uplouban);
                #region 画标高          
                Point3d pointbioagao = point + new Vector3d(data.neiqiangkuan + 900 + 300, totalHeight + data.dibanhou + data.dianceng - dingduanjuli, 0);
                //线
                Line biaogaodixian = new Line(pointbioagao + new Vector3d(-200, 0, 0), pointbioagao + new Vector3d(200, 0, 0));
                Line biaogaoshangxian = new Line(pointbioagao + new Vector3d(-200, 200, 0), pointbioagao + new Vector3d(1000, 200, 0));
                Line biaogaozuoxiexian = new Line(pointbioagao, pointbioagao + new Vector3d(200, 200, 0));
                ...
                MText mtext0 = new MText();//文字
                mtext0.SetDatabaseDefaults();
                mtext0.Contents = data.storyList[i - 1].StoryElevation;
                mtext0.TextHeight = 250;
                mtext0.Attachment = AttachmentPoint.MiddleCenter;// 设置文本对象的插入点为文本的顶端中点
                mtext0.Location = pointbioagao + new Vector3d(600, 400, 0);
                mtext0.Layer = "0S-BEAM-TEXT";
                ...
                //楼板的下边那条线
                Line downlouban = new Line(
                    point + new Vector3d(data.neiqiangkuan, totalHeight + data.dibanhou + data.dianceng - dingduanjuli - banhou, 0),
                    point + new Vector3d(data.neiqiangkuan + 900, totalHeight + data.dibanhou + data.dianceng - dingduanjuli - banhou, 0));
                downlouban.Layer = "0S-WALL-LINE";
                downloubanlist.Add(downlouban);
                //板厚标注
                Point3d zhongjianloubanbiaozhu1 = point + new Vector3d(data.neiqiangkuan + 450, totalHeight + data.dibanhou + data.dianceng - dingduanjuli, 0);
                Point3d zhongjianloubanbiaozhu2 = point + new Vector3d(data.neiqiangkuan + 450, totalHeight + data.dibanhou + data.dianceng - dingduanjuli - banhou, 0);
                OutWall.DrawBiaoZhu(db, zhongjianloubanbiaozhu1, zhongjianloubanbiaozhu2, 0);
                //折断线
                Polyline zdx = ZheDuanXian(point + new Vector3d(data.neiqiangkuan + 900, totalHeight + data.dibanhou + data.dianceng - dingduanjuli - banhou, 0),
                            point + new Vector3d(data.neiqiangkuan + 900, totalHeight + data.dibanhou + data.dianceng - dingduanjuli, 0));
                zhongjianzdx.Add(zdx);
                //楼板处的附加筋
                double shangmaogu = Double.Parse(data.storyList[i - 1].StoryHeight) / 3;
                double xiamaogu = Double.Parse(data.storyList[i].StoryHeight) / 3;
                Vector2d vector2Dtemp = new Vector2d(point.X, point.Y);
                Polyline fujia = new Polyline();
                fujia.AddVertexAt(0, new Point2d(40 + 30 + 60, totalHeight + data.dibanhou + data.dianceng - dingduanjuli - banhou - xiamaogu + 60) + vector2Dtemp, 0, 0, 0);
                fujia.AddVertexAt(1, new Point2d(40 + 30, totalHeight + data.dibanhou + data.dianceng - dingduanjuli - banhou - xiamaogu) + vector2Dtemp, 0, 0, 0);
                fujia.AddVertexAt(2, new Point2d(40 + 30, totalHeight + data.dibanhou + data.dianceng - dingduanjuli + shangmaogu) + vector2Dtemp, 0, 0, 0);
                fujia.AddVertexAt(3, new Point2d(40 + 30 + 60, totalHeight + data.dibanhou + data.dianceng - dingduanjuli + shangmaogu - 60) + vector2Dtemp, 0, 0, 0);
                fujia.Closed = false;
                fujia.Layer = "0S-DETL-RBAR";
                fujia.ConstantWidth = 10;
                fujiajin.Add(fujia);
                //附加筋标注
                Point3d fujiajinbiaozhu1 = point + new Vector3d(40 + 30, totalHeight + data.dibanhou + data.dianceng - dingduanjuli - banhou - xiamaogu, 0);
                Point3d fujiajinbiaozhu2 = point + new Vector3d(40 + 30, totalHeight + data.dibanhou + data.dianceng - dingduanjuli + shangmaogu, 0);
                OutWall.DrawBiaoZhu(db, fujiajinbiaozhu1, fujiajinbiaozhu2, 500);
            }
            #endregion
            ...
            #region 画素砼垫层
            Line diancengup = new Line(
                point + new Vector3d(0, data.dianceng, 0),
                point + new Vector3d(data.neiqiangkuan + 900, data.dianceng, 0));
            diancengup.Layer = "0S-WALL-LINE";
            Line diancengdown = new Line(
                point + new Vector3d(0, 0, 0),
                point + new Vector3d(data.neiqiangkuan + 900, 0, 0));
            diancengdown.Layer = "0S-WALL-LINE";
            //“素砼垫层”
            MText mtextSTDC = new MText();//数字
            mtextSTDC.SetDatabaseDefaults();
            ...
            mtextSTDC.TextHeight = 200;
            mtextSTDC.Attachment = AttachmentPoint.BottomCenter;// 设置文本对象的插入点为文本的顶端中点
            mtextSTDC.Location = point + new Vector3d((data.neiqiangkuan + 900) * 0.75, -400, 0);
            mtextSTDC.Layer = "0S-BEAM-TEXT";
            //引线
            Polyline stdc = new Polyline();
            stdc.AddVertexAt(0, new Point2d(point.X + (data.neiqiangkuan + 900) / 2 - 100, point.Y), 0, 0, 0);
            stdc.AddVertexAt(1, new Point2d(point.X + (data.neiqiangkuan + 900) / 2, point.Y - 400), 0, 0, 0);
            stdc.AddVertexAt(2, new Point2d(point.X + data.neiqiangkuan + 900, point.Y - 400), 0, 0, 0);
            stdc.Closed = false;
            stdc.Layer = "0S-WALL-DIMS";
            //标注
            Point3d diancengbiaozhudian1 = point + new Vector3d(data.neiqiangkuan + 900, data.dianceng, 0);
            Point3d diancengbiaozhudian2 = point + new Vector3d(data.neiqiangkuan + 900, 0, 0);
            OutWall.DrawBiaoZhu(db, diancengbiaozhudian1, diancengbiaozhudian2, 500);

            #region 填充的边界
            // 添加边界路径
            Polyline boundary = new Polyline();
            boundary.AddVertexAt(0, new Point2d(point.X, point.Y + data.dianceng), 0, 0, 0);
            boundary.AddVertexAt(1, new Point2d(point.X + data.neiqiangkuan + 900, point.Y + data.dianceng), 0, 0, 0);
            boundary.AddVertexAt(2, new Point2d(point.X + data.neiqiangkuan + 900, point.Y), 0, 0, 0);
            boundary.AddVertexAt(3, new Point2d(point.X, point.Y), 0, 0, 0);
            boundary.AddVertexAt(4, new Point2d(point.X, point.Y + data.dianceng), 0, 0, 0);
            boundary.Closed = true;
            ObjectIdCollection boundaryIds = new ObjectIdCollection();
            ...
            //外层钢筋
            Polyline waitongchang = new Polyline();
            waitongchang.AddVertexAt(0, new Point2d(neiqiangkuan0 + 900 - 70, data.dianceng + 20) + vector2D, 0, 0, 0);
            waitongchang.AddVertexAt(1, new Point2d(40, data.dianceng + 20) + vector2D, 0, 0, 0);
            waitongchang.AddVertexAt(2, new Point2d(40, totalHeight + data.dibanhou + data.dianceng - 20) + vector2D, 0, 0, 0);
            waitongchang.AddVertexAt(3, new Point2d(40 + 60, totalHeight + data.dibanhou + data.dianceng - 20) + vector2D, 0, 0, 0);
            waitongchang.Closed = false;
            waitongchang.Layer = "0S-DETL-RBAR";
            waitongchang.ConstantWidth = 10;
            //内侧钢筋
            Polyline neitongchang = new Polyline();
            neitongchang.AddVertexAt(0, new Point2d(neiqiangkuan0 + 900 - 70, data.dianceng + 80) + vector2D, 0, 0, 0);
            neitongchang.AddVertexAt(1, new Point2d(neiqiangkuan0 - 40, data.dianceng + 80) + vector2D, 0, 0, 0);
            neitongchang.AddVertexAt(2, new Point2d(neiqiangkuan0 - 40, totalHeight + data.dibanhou + data.dianceng - 20) + vector2D, 0, 0, 0);
            neitongchang.AddVertexAt(3, new Point2d(neiqiangkuan0 - 40 + 60, totalHeight + data.dibanhou + data.dianceng - 20) + vector2D, 0, 0, 0);
            neitongchang.Closed = false;
            neitongchang.Layer = "0S-DETL-RBAR";
            neitongchang.ConstantWidth = 10;
            //底板处的附加筋
            double maogu = Double.Parse(data.storyList[data.storyList.Count - 1].StoryHeight) / 3;
            Polyline dibufujia = new Polyline();
            dibufujia.AddVertexAt(0, new Point2d(neiqiangkuan0 + 900 - 70, data.dianceng + 50) + vector2D, 0, 0, 0);
            dibufujia.AddVertexAt(1, new Point2d(40 + 30, data.dianceng + 50) + vector2D, 0, 0, 0);
            dibufujia.AddVertexAt(2, new Point2d(40 + 30, maogu + data.dibanhou + data.dianceng) + vector2D, 0, 0, 0);
            dibufujia.AddVertexAt(3, new Point2d(40 + 30 + 60, maogu + data.dibanhou + data.dianceng - 60) + vector2D, 0, 0, 0);
            dibufujia.Closed = false;
            dibufujia.Layer = "0S-DETL-RBAR";
            dibufujia.ConstantWidth = 10;
            //附加筋标注
            Point3d dibanfujiajinbiaozhu1 = point + new Vector3d(40 + 30, data.dibanhou + data.dianceng, 0);
            Point3d dibanfujiajinbiaozhu2 = point + new Vector3d(40 + 30, maogu + data.dibanhou + data.dianceng, 0);
            OutWall.DrawBiaoZhu(db, dibanfujiajinbiaozhu1, dibanfujiajinbiaozhu2, 500);
            #endregion
    ...
    public class WallData
    {
        ...
        /// 垫层  
        /// </summary>
        public double dianceng { get; set; }

        /// <summary>
        /// 顶标高
        /// </summary>
        public double dingbiaogao { get; set; }

        /// <summary>
        /// 内墙厚
        /// </summary>
        public double neiqiangkuan { get; set; }

        /// <summary>
        /// 线内墙厚
        /// </summary>
        public double xianeiqiangkuan { get; set; }

        /// <summary>
        /// 外墙厚
        /// </summary>
        public double waiqiangkuan { get; set; }

        /// <summary>
        /// 垫板厚
        /// </summary>
        public double dibanhou { get; set; }
